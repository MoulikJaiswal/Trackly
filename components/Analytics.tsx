        ...
        return (
          <div key={d.date} className="flex flex-col items-center gap-2 flex-1 group">
             <div className="relative w-full flex justify-center h-full items-end">
                ...
                {/* Bar */}
                <div 
                   className={`w-full max-w-[24px] md:max-w-[40px] rounded-t-lg transition-all duration-700 ease-out relative overflow-hidden ${isToday ? 'bg-indigo-500' : 'bg-slate-200 dark:bg-slate-800 group-hover:bg-indigo-500/50'}`}
          ...
          <div key={sub.id} className="flex items-center gap-4 bg-slate-50 dark:bg-black/20 p-3 rounded-2xl border border-slate-200 dark:border-white/5">
             ...
             <div className="flex-grow min-w-0">
               ...
               <div className="h-1.5 w-full bg-slate-200 dark:bg-slate-800 rounded-full overflow-hidden">
                 <div 
                    className={`h-full ${sub.bg} transition-all duration-1000`} 
         ...
         <div className="lg:col-span-2 space-y-6 md:space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
               ...
               <div className="space-y-4">
                   ...
                   <div className="p-6 bg-gradient-to-br from-indigo-50 to-purple-50 dark:from-indigo-500/10 dark:to-purple-500/5 rounded-3xl border border-indigo-100 dark:border-indigo-500/20 relative overflow-hidden h-auto md:h-full flex flex-col justify-center min-h-[160px]">
            ...
            <Card className="bg-white/60 dark:bg-slate-900/60 p-6 h-full">
              ...
              <div className="space-y-5">
                {MISTAKE_TYPES.map(type => {
                  ...
                  const percent = totalMistakes > 0 ? (count / totalMistakes) * 100 : 0;
                  ...
                  return (
                    <div key={type.id} className="group">
                      ...
                      <div className="h-2 w-full bg-slate-200 dark:bg-black/40 rounded-full overflow-hidden">
                        <div 
                          className={`h-full ${type.color.replace('text', 'bg').replace('-400', '-500 dark:bg-' + type.color.split('-')[1] + '-400')} transition-all duration-1000 ease-out`}
                          style={{ width: `${Math.max(percent, totalMistakes > 0 ? 5 : 0)}%` }}
                        />
                      </div>
                    </div>
                  );
