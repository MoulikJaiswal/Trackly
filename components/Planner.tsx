...
export const Planner: React.FC<PlannerProps> = ({ targets, onAdd, onToggle, onDelete }) => {
  ...
  return (
    <div className="space-y-6 md:space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      {/* Planner Controls - Segmented Control Style */}
      <div className="flex justify-between items-center bg-white/60 dark:bg-slate-900/40 backdrop-blur-xl p-1.5 rounded-2xl border border-slate-200 dark:border-white/10 shadow-sm">
          {/* Segmented Toggle */}
          <div className="flex p-1 bg-slate-100 dark:bg-black/20 rounded-xl">
              <button 
                onClick={() => setViewMode('week')}
                className={`w-24 py-2 rounded-lg text-xs font-bold uppercase tracking-wider transition-all ${viewMode === 'week' ? 'bg-white dark:bg-indigo-600 text-indigo-600 dark:text-white shadow-sm' : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300'}`}
              >
                  Week
              </button>
              <button 
                onClick={() => setViewMode('month')}
                className={`w-24 py-2 rounded-lg text-xs font-bold uppercase tracking-wider transition-all ${viewMode === 'month' ? 'bg-white dark:bg-indigo-600 text-indigo-600 dark:text-white shadow-sm' : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300'}`}
              >
                  Month
              </button>
          </div>
          ...
          <button 
             ...
             className="px-4 py-2 mr-1 bg-indigo-500/10 dark:bg-indigo-500/20 text-indigo-600 dark:text-indigo-300 text-[10px] font-bold uppercase tracking-wider rounded-xl hover:bg-indigo-600 hover:text-white transition-colors"
           >
             Go to Today
           </button>
      </div>

      <div className="bg-white/60 dark:bg-slate-900/40 backdrop-blur-xl border border-slate-200 dark:border-white/10 rounded-3xl p-4 shadow-xl select-none transition-colors">
             ...
             <div className="grid grid-cols-7 gap-1 md:gap-2">
                {weekDays.map((dateStr) => {
                    ...
                    >
                        ...
                        <span className={`text-base md:text-lg font-mono font-bold ${isSelected ? 'text-white' : 'text-slate-700 dark:text-slate-200'}`}>{d.getDate()}</span>
                        
                        <div className="flex gap-0.5 mt-2">
                            {hasTest && <div className={`w-1.5 h-1.5 rounded-full ${isSelected ? 'bg-amber-300' : 'bg-amber-500'}`} />}
                            {!hasTest && tasksForDay.length > 0 && (
                                <div className={`w-1.5 h-1.5 rounded-full ${allDone ? 'bg-emerald-400' : isSelected ? 'bg-indigo-300' : 'bg-indigo-500/50'}`} />
                            )}
                        </div>
                    </button>
                    )
                        ...
                        return (
                            ...
                            >
                                ...
                                <div className="flex gap-0.5 mt-1">
                                    {hasTest && <div className={`w-1 h-1 rounded-full ${isSelected ? 'bg-amber-300' : 'bg-amber-500'}`} />}
                                    {!hasTest && hasTasks && <div className={`w-1 h-1 rounded-full ${isSelected ? 'bg-indigo-300' : 'bg-indigo-500'}`} />}
                                </div>
                            </button>
                        );
                    })}
      ...
      <Card className="min-h-[400px]">
        ...
        <div className="space-y-3">
          ...
          {dayTargets.length === 0 ? (
             <div className="text-center py-16 opacity-40 border-2 border-dashed border-slate-200 dark:border-white/5 rounded-3xl">
               <Target size={40} className="mx-auto mb-4 text-slate-400 dark:text-slate-600"/>
            ...
            dayTargets.map(t => (
              <div key={t.id} className={`flex items-center gap-4 p-3 md:p-4 rounded-2xl group border transition-all ${t.type === 'test' ? 'bg-amber-50 dark:bg-amber-500/5 border-amber-200 dark:border-amber-500/10' : 'bg-slate-50 dark:bg-white/[0.03] border-slate-100 dark:border-white/5 hover:border-slate-300 dark:hover:border-white/10'}`}>
                ...
                <div className="flex-grow flex flex-col">
                    <span className={`text-sm transition-all duration-300 ${t.completed ? 'text-slate-400 dark:text-slate-500 line-through' : 'text-slate-700 dark:text-slate-200'}`}>
                        {t.text}
                    </span>
                    {t.type === 'test' && (
